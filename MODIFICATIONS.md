...
### 怎么让内核认识这些节点(以及为什么最后没走 profile 这条路)

`delayProxyByName` 走的是 mihomo 的 `/proxies/{name}/delay`,**只能测配置里已经加载的
节点**。免费池不在你的订阅里,所以得先塞进去。

第一版打算用 Clash Verge Rev 自带的 **`proxies` 类型 profile**:文件内容是
`{prepend, append, delete}`,`enhance/seq.rs` 的 `use_seq` 会把 `append` 里的节点
**追加**到运行时 `proxies` 列表(不是替换),`enhance_profiles` 让内核重载。
这是上游的一等机制,能做到。**但最后没走这条路** —— 见下一节。

排查过程中确认了一件重要的事,记在这儿备查:**普通 `merge` profile 不能用**。
`enhance/merge.rs` 的 `deep_merge` 对非 mapping 一律 `*a = b`,也就是写 `proxies:`
会把用户真正的节点**整个替换掉**。`merge.rs` 的测试夹具里出现了 `append-proxies`,
但那只是夹具 —— 实现里没有对应处理,而且那个测试根本没断言(`let _ = ...`)。
真正处理 prepend/append 的是 `seq.rs`,只对 `rules` / `proxies` / `groups`
三种专用 profile 类型生效。
...
## 改动内容(2026-09-09):免费节点池的实测(第二步:独立探针进程 + 界面)

### 为什么另起一个进程,而不是往用户的配置里塞

上一节说了,用 `proxies` 类型 profile 把节点追加进运行时配置是能做到的。真去做的
时候几件事加在一起,结论反过来了:

- `use_seq` 追加之后会把这些名字**塞进第一个 selector 策略组**(seq.rs:92 起)。
  几十上百条来路不明的节点直接出现在代理选择列表里。
- 重载配置那一下**正在走的连接会断**。测一批免费节点不值得让家里正在看的视频卡一下。
- 中途出任何岔子 —— 崩溃、断电、用户点了别的 —— 这些节点就**留在配置里**了。
  清理逻辑要考虑的状态比测试本身还多。

所以改成单独起一个 mihomo:

```yaml
mixed-port: 0                              # 不监听任何代理端口,不可能跟主内核抢
external-controller: 127.0.0.1:<随机端口>   # 只开这一个口
secret: <随机>
proxies: [...]                             # 待测的那一批
proxy-groups: []
rules: [MATCH,DIRECT]
...
用户手里那份能上网的配置全程没被碰过。进程管理在 `src-tauri/src/cmd/probe.rs`:
`probe_start` / `probe_stop` / `probe_running` 三个命令,`probe_stop` 是幂等的,
`RunEvent::Exit` 里还会兜底杀一次 —— **留一个孤儿 mihomo 在系统里是这个功能最讨厌的
失败方式**:它占着端口、还连着网,而用户完全看不见。前端那一层也把整个测试循环包在
`try/finally` 里,不管哪一步炸都会 `probe_stop`。

### 配置片段用 js-yaml 生成,不手拼字符串

免费节点的密码、路径、名字是从公开仓库抓来的,引号、冒号、井号、反斜杠什么都有。
手拼一定会在某条数据上翻车,而**翻车的表现是整个探针起不来**,报错只会说"内核没起来",
看不出是哪一条的问题。

`buildProbeYaml()` 的测试专门钉了一个 YAML 的经典坑:`password: yes` 会被解析成
布尔 `true`,`password: 0755` 会被解析成数字。降级之后 mihomo 拿到的就不是原来那个
密码了,节点必然连不上,**而它会被记成"测过了,不通"** —— 数据被污染,还查不出原因。

### 界面

`src/components/deno-push/free-pool-verify.tsx`,Deno Push 页的第三块。单独一个组件
而不是塞进页面:它有自己一套状态(轮次、进度、历轮汇总),混进去的话上面搜索框每敲
一个字符都会带着它一起重渲。

一轮上限给到 300 条。每条最多 5 秒、并发 8 的话 300 条大约三五分钟,期间这台机器一直
在往一堆陌生服务器发包。想测更多就多跑几轮 —— 服务端每轮独立记账,分几次跟一次测完
是一样的。

界面上写死了一句话:**测出「通」不等于能给家人用**。这一步只回答"这条链接现在还活着
吗",要用还得走「扫描 → 筛美国 → 推送」,中间有 GeoIP 和可达性两道关。免费节点是
陌生人在跑的,任何时候都不能直接推给订阅。

服务端那边同时加了 `GET /free/verify`(历轮通过率),界面上列出来。单看一轮说明不了
什么 —— 免费池本来就是通一半坏一半;要看的是趋势:一路往下说明源在烂掉,突然掉到零
那多半是本机网络的问题。

### 顺带

CI 的 `test` job 加了一步 `pnpm typecheck`。`build` 里的 `web:build` 也会跑 `tsc`,
但那要等二十分钟的 Rust 编译,而且四个平台各报一遍同一个错。这次就是先在测试文件里
写出了一个 `TS2339` 才想起来加。
